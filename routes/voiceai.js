...
const createAgentJSon = require("../utils/constants");
...
router.post("/assistant", authenticateUser, async (req, res) => {
  ...
  if (!name || !prompt) {
    return res.status(400).json({ message: "Both name and prompt are required" });
  }
  ...
  try {
    ...
    const vapiResponse = await axios.post("https://api.vapi.ai/assistant", payload, {
      headers: {
        Authorization: `Bearer ${VAPI_API_KEY}`,
